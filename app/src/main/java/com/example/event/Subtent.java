...
public class Subtent extends AppCompatActivity {
    ...
    DatabaseReference tent;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_subtent);

        ActionBar actionBar = getSupportActionBar();
        actionBar.setTitle("Details");

        database = FirebaseDatabase.getInstance();
        tent = database.getReference("Tent");
        ...
        if (getIntent() != null)
            tentId = getIntent().getStringExtra("Id");
        if(!tentId.isEmpty())
        {
            getDetailTent(tentId);
        }
    ...
    private void getDetailTent(String tentId)
    {

       tent.child(tentId).addValueEventListener(new ValueEventListener() {
            @Override
